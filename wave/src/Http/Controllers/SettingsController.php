...
class SettingsController extends Controller
{
    ...
    public function profilePut(Request $request)
    {
        $request->validate(
            ...
            [
                'avatar.base64image' => 'Avatar harus menjadi gambar yang valid.'
            ]
        );
        ...
        if ($request->bukti_pembayaran) {
            $authed_user->bukti_pembayaran = $this->saveBuktiPembayaran($request->bukti_pembayaran, $authed_user->username);
        }
        $authed_user->save();
        ...
        return back()->with(['message' => 'Profil pengguna berhasil diperbarui!', 'message_type' => 'success']);
    }
    ...
    private function saveBuktiPembayaran($bukti_pembayaran, $filename)
    {
        $path = 'tempat_bukti_pembayaran/' . $filename . '.png';
        Storage::disk(config('voyager.storage.disk'))->put($path, file_get_contents($bukti_pembayaran));
        return $path;
    }
