                ...
                <div class="col-md-8">
                    <div class="panel panel-bordered">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="no_hp_camaba">{{ __('Nomor Handphone CAMABA') }}</label>
                                <input type="text" class="form-control" id="no_hp_camaba" name="no_hp_camaba"
                                    placeholder="{{ __('no whatsapp') }}"
                                    value="@if (isset($dataTypeContent->no_hp_camaba)) {{ $dataTypeContent->no_hp_camaba }} @endif">
                            </div>

                            <div class="form-group">
                                <label for="no_hp_ortu">{{ __('Nomor Handphone Ortu') }}</label>
                                <input type="text" class="form-control" id="no_hp_ortu" name="no_hp_ortu"
                                    placeholder="{{ __('no whatsapp') }}"
                                    value="@if (isset($dataTypeContent->no_hp_ortu)) {{ $dataTypeContent->no_hp_ortu }} @endif">
                            </div>

                            <div class="form-group">
                                <label for="bukti_pembayaran">{{ __('Bukti Pembayaran Administrasi') }}</label>
                                @if (isset($dataTypeContent->bukti_pembayaran))
                                    <img src="{{ filter_var($dataTypeContent->bukti_pembayaran, FILTER_VALIDATE_URL) ? $dataTypeContent->bukti_pembayaran : Voyager::image($dataTypeContent->bukti_pembayaran) }}"
                                ...
                                <div class="form-group">
                                    <label for="status_pembayaran">{{ __('Status Pembayaran') }}</label>
                                    <input type="checkbox" id="status_pembayaran" name="status_pembayaran">
                                </div>
                                    ...
                                    <select name="role_id" id="role_id" class="select2" placeholder="">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}"
                                                @if ($role->id == $dataTypeContent->role_id) selected @endif>{{ $role->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
