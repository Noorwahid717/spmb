...
<div class="flex flex-col px-8 mx-auto my-6 lg:flex-row max-w-7xl xl:px-5">
    <div class="flex flex-col justify-start flex-1 overflow-hidden bg-white border rounded-lg lg:ml-3 border-gray-150">
        <div class="flex flex-wrap items-center justify-between p-5 bg-white border-b border-gray-150 sm:flex-no-wrap">
            ...
            <div class="relative flex-1">
                <h3 class="text-lg font-medium leading-6 text-gray-700">
                    Pengelolaan Ujian Hafalan Shalawat Wahidiyah
                </h3>
                <p class="text-sm leading-5 text-gray-500 mt">
                    Kelola Ujian Hafalan Shalawat Wahidiyah Tahun Akademik <strong>{{$ta_long}}</strong> Nama Gelombang
            ...
            <a class="inline-flex self-start items-center ml-3 px-4 py-3 bg-wave-400 hover:bg-wave-600 text-white text-sm font-medium rounded-md"
                data-modal="#addExamReadShalawatModal" rel="modal:open" href="#addExamReadShalawatModal"
        ...
        <div class="relative p-5">
            ...
            <table id="exam_read_shalawat" class="display exam_read_shalawat" style="width:100%;">
                <thead>
                    <tr>
            ...
            <input type="hidden" id="exam_read_shalawat_url" class="exam_read_shalawat_url"
                name="exam_read_shalawat_url" value="{{route('wave.exam-read-shalawat-getlist')}}">
            <input type="hidden" id="detele_exam_read_shalawat_url" class="detele_exam_read_shalawat_url"
                name="detele_exam_read_shalawat_url" value="{{route('wave.exam-read-shalawat-delete')}}">
            <input type="hidden" id="exam_read_shalawat_reset_url" class="exam_read_shalawat_reset_url"
                name="exam_read_shalawat_reset_url" value="{{route('wave.exam-read-shalawat-reset')}}">
            @include('theme::seleksi.exam_read_shalawat.modal.add')
            @include('theme::seleksi.exam_read_shalawat.modal.edit')
            @include('theme::seleksi.exam_read_shalawat.modal.detail-mhs')
        </div>
    </div>
</div>
...
<script>
    ...
    var table = $('.exam_read_shalawat').DataTable({
        pageLength : 10,
        dom: 'lfrtip',        
        processing: true,
        serverSide: true,
        ...
        rowId:  'id',
        ajax: {
            url:$('#exam_read_shalawat_url').val(),
            type:"POST",
            data:function(d){
...
<script>
    function deleteModalClick(id,id_penguji,sesi,penguji,ta,gelombang){        
        const contents = `Anda akan menghapus ujian hafalan shalawat wahidiyah berikut: ${ta} ${gelombang} <br>Penguji <strong>${penguji} : ${sesi}</strong>`;          
        Swal.fire({
            ...
            reverseButtons: true
                ...
                if (result.isConfirmed) {  
                    deleteExamReadShalawat(id);
                } 
        });
    }
    function deleteExamReadShalawat(id) {
        $('.containerr').show();
        let datar = {};
        datar['_method']='POST';
        ...
        $.ajax({
            type: 'post',
            url: $("#detele_exam_read_shalawat_url").val(),
            data:datar,
...
<script>
    ...
    var tableAvailable = $('.available_member').DataTable({
        ...
        ajax: {
            ...
            data:function(d){
                ...
                d.ta_long = @json($ta_long);
                d.id_exam_read_shalawat = $('#id_exam_read_shalawat').val();           
    ...
    function setIdExamReadShalawatModal(id,tanggal,waktu,tempat,nama_penguji,sesi){
        $('#id_exam_read_shalawat').val(id);
...
<script>
    function resetHasilUjianReadShalawat(id_exam_read_shalawat_member,id_camaba,nama,prodi,ta_seleksi) {
        const contents = `Anda akan mereset hasil ujian hafalan Shalawat Wahidiyah berikut: <strong>${nama} - ${prodi} - ${ta_seleksi}</strong>`;          
        Swal.fire({
            title: 'Apakah anda yakin!',
            // text: teks,
            html: contents,
            ...
            reverseButtons: true
                ...
                if (result.isConfirmed) {  
                    doResetHasilUjianReadShalawat(id_exam_read_shalawat_member,id_camaba,ta_seleksi);
                } 
        });
    }

    function doResetHasilUjianReadShalawat(id_exam_read_shalawat_member,id_camaba,ta_seleksi){
        $('.containerr').show();
        let params = {id_exam_read_shalawat_member,id_camaba,ta_seleksi};
        let header = {};
        header['_method']='POST';
        ...
        $.ajax({
            type: 'post',
            url: $("#exam_read_shalawat_reset_url").val(),
            data:datar,
            success: function(data) {
                if (data.error==false) {
                    $('.containerr').hide();                    
    ...
    function editModalClick(id,id_penguji,nama_sesi,tanggal,waktu,tempat){
        $('#edit_id_exam_read_shalawat').val(id);
        $("#edit_daftar_penguji").val(id_penguji);
        $("#old_session_name").val(nama_sesi);
        $("#edit_session_name").val(nama_sesi);
        $("#edit_tanggal_read_shalawat").val(tanggal);
        $("#edit_waktu_read_shalawat").val(waktu.substring(0, 5));
        $("#edit_tempat_read_shalawat").val(tempat);
    }

    function editExamReadShalawat(){
        const id = $("#edit_id_exam_read_shalawat").val();
        ...
        const tanggal_read_shalawat = $("#edit_tanggal_read_shalawat").val();
        const waktu_read_shalawat = $("#edit_waktu_read_shalawat").val();
        const tempat_read_shalawat = $("#edit_tempat_read_shalawat").val();
        const params = {
            id,
            id_exam_schedule,
            id_penguji,
            old_session_name,
            session_name,
            tanggal_read_shalawat,
            waktu_read_shalawat,
            tempat_read_shalawat
        };

        if(id_penguji!="-1"){                        
        if(session_name!=""){
        if(tanggal_read_shalawat!=""){
        if(waktu_read_shalawat!=""){
        if(tempat_read_shalawat){
            updateExamReadShalawat(params);                            
        }else{
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: "Tempat ujian hafalan shalawat wahidiyah wajib diisi!",
            });
        }
        }else{
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: "Waktu ujian hafalan shalawat wahidiyah wajib diisi!",
            });
        }
        }else{
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: "Tanggal ujian hafalan shalawat wahidiyah wajib diisi!",
            });
        }
        }else{
            Swal.fire({
    ...
    function updateExamReadShalawat(params){
        $('.containerr').show();
        let header = {};
        header['_method']='POST';
        header['_token']=$('._token').data('token');
        ...
        $.ajax({
            type: 'post',
            url: $("#editExamReadShalawatUrl").val(),
            data:datar,
            success: function(data) {
                if (data.error==false) {
                    $('.containerr').hide();                    
    ...
    function modalAddClick(){
        ...
        $("#session_name").val("");
        $("#tanggal_read_shalawat").val("");
        $("#waktu_read_shalawat").val("");
        $("#tempat_read_shalawat").val("");
    }

    function addExamReadShalawat(){
        ...
        const tanggal_read_shalawat = $("#tanggal_read_shalawat").val();
        const waktu_read_shalawat = $("#waktu_read_shalawat").val();
        const tempat_read_shalawat = $("#tempat_read_shalawat").val();
        const params = {
            id_exam_schedule,
            id_penguji,
            session_name,
    ...
    function saveExamReadShalawat(params){
        $('.containerr').show();
        let header = {};
        header['_method']='POST';
        header['_token']=$('._token').data('token');
        ...
        $.ajax({
            type: 'post',
            url: $("#addExamReadShalawatUrl").val(),
            data:datar,
            success: function(data) {
                if (data.error==false) {
                    $('.containerr').hide();                    
    ...
    function addMemberClick(id_exam_read_shalawat,id_camaba,ta_seleksi){
        $('.containerr').show();
        let params = {id_exam_read_shalawat,id_camaba,ta_seleksi};
        let header = {};
        header['_method']='POST';
        header['_token']=$('._token').data('token');
        let datar = {...header,...params};
        ...
        $.ajax({
            type: 'post',
            url: $("#addReadShalawatMemberUrl").val(),
            data:datar,
            success: function(data) {
                if (data.error==false) {
                    $('.containerr').hide();                    
    ...
    function deleteMemberClick(id_exam_read_shalawat,id_camaba,ta_seleksi){
        $('.containerr').show();
        let params = {id_exam_read_shalawat,id_camaba,ta_seleksi};
        let header = {};
        header['_method']='POST';
        ...
        $.ajax({
            type: 'post',
            url: $("#deleteReadShalawatMemberUrl").val(),
            data:datar,
            success: function(data) {
                if (data.error==false) {
                    $('.containerr').hide();                    
