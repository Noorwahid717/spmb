...
<input type="hidden" name="saveOrUpdateUrlStep5" id="saveOrUpdateUrlStep5"
    value="{{route('wave.validasi-pendaftaran-update-data-riwayat-pendidikan')}}">
...
@else
@if($step_5!=null&&$step_5->last_note!=null&&$step_5->last_note!="")
<div class="note-error mb-5 py-2">
    <strong>
        HISTORI CATATAN VALIDASI SEBELUMNYA !!!
    </strong>
    <p>{{$step_5->last_note}}</p>
</div>
...
<div class="flex flex-col px-3 mx-auto my-6 lg:flex-row max-w-7xl xl:px-5">
    <div class="flex flex-col justify-start flex-1 mb-5 xl:px-5 md:px-2 overflow-hidden bg-white">
        <div class="form-group mb-5 text-xs mb-4">
            <label for="note_step_5">Catatan Validasi</label>
            <textarea name="note_step_5" id="note_step_5" cols="30" rows="1" class="form-control mt-1"></textarea>
        </div>
    </div>
    <div class="flex flex-col justify-start flex-1 mb-5 xl:px-5 md:px-2 overflow-hidden bg-white">
        <div class="form-group mb-5 text-xs mb-4">
            <label for="lock_step_5">Status Validasi Data Alamat ?</label>
            <select name="lock_step_5" id="lock_step_5" class="form-control mt-1">
